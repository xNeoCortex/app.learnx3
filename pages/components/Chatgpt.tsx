...
function ChatGpt() {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      setResultGPT(
        response.data.choices[0]?.message?.content ?? "No message returned"
      )
      setLoading(false)
    } catch (error) {
      console.error(error)
      ...
      >
        {resultGPT.length > 0 && (
          <div
            ...
            onClick={() => (
              setResultGPT(""), setAbbreviation(""), setCustomAbbrev([])
            )}
          >
            X
          </div>
        ...
        <div style={{ margin: 10 }}>
          ...
          <input type="text" id="name" name="name" value={abbreviation} />
        </div>
        <div
          style={{
            width: "100%",
        ...
        >
          {(customAbbrev.length > 0 || resultGPT.length > 0 || loading) && (
          ...
          {loading
            ...
            ? JSON.parse(resultGPT?.replace(/'/g, '"')).map(
                (item: any, index: number) => (
                  ...
                  >
                    {`${item}${
                      index + 1 !==
                      JSON.parse(resultGPT?.replace(/'/g, '"')).length
                        ? ","
                        : " "
                    }`}
                  </span>
