...
export default function MultipleSelectAssessment({
  selectedLessons,
  setSelectedLessons,
  assessmentType,
}) {
  ...
  // Fetching lessons
  const { data, isLoading, isError } = useQuery([assessmentType], () =>
    fetchAssessment(assessmentType)
  )
    ...
    <FormControl sx={{ mt: 3, width: "100%" }} fullWidth>
      ...
      <Select
        ...
        renderValue={(selected) =>
          selected.map((item) => item.topic).join(", ")
        }
        MenuProps={MenuProps}
      >
        {data?.data?.length > 0 &&
