...
import { useStoreFilePath } from "../zustand"
...
export function Avatar(props) {
	const { aiFilePath } = useStoreFilePath()
	const [script, setScript] = useState("welcome")

	const audio = useMemo(() => new Audio(aiFilePath), [aiFilePath])
	const jsonFile = useLoader(THREE.FileLoader, `audios/${script}.json`)
	const lipsync = JSON.parse(jsonFile)

	const [morphTargetSmoothing, setMorphTargetSmoothing] = useState(0.5)
	const [smoothMorphTarget, setSmoothMorphTarget] = useState(false)
