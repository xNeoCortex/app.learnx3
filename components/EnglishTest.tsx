			...
			<Grid container spacing={2}>
				{uniqueTests.map((test, index) => (
					<Grid item xs={4} key={index}>
						...
						>
							<CardMedia component="img" height="194" image="/test_imag.png" alt="test image" />
							...
							>
								<Typography sx={{ fontWeight: 600, fontSize: "14px" }}>
									{test?.topic.length < 40
										? test?.topic.split(" ")[0].slice(0, 1).toUpperCase() + test?.topic.slice(1)
										: test?.topic.split(" ")[0].slice(0, 1).toUpperCase() + test?.topic.slice(1, 40) + "..."}
								</Typography>
								<Link href={`/test/${test.topic_id}`}>
									<Button
										variant="contained"
										size="small"
										sx={{
